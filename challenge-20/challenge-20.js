(function(win, doc){
    ...
    */
    ...
    `$inputEmail`.
    */
    var $inputEmail = doc.getElementsByTagName('input')[1];
    ...
    */
    ...
    $button.addEventListener('click', function(event){
        ...
        if(!isFill($inputUsername)) alert('Preencha o nome do usuário!');
        if(!isFill($inputEmail)) alert('Preencha o e-mail!')
        else if(!isValidEmail($inputEmail.value))
            alert('Entre com um e-mail válido!');

        if(!isFill($message)) alert('Preencha a mensagem!');
        if(isFill($inputUsername) && isValidEmail($inputEmail.value) && isFill($message))
    ...
    */
    function isValidEmail(email){
        return email.match(/^[\w.+]+[.+\w]+@\w+.\w{3}.\w{2,3}$/);
    }
})(window, document);
